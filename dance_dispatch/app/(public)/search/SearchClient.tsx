...
function eventMatchesQuery(event: any, query: string, venue: any): boolean {
    const eventText = [
        event.title,
        event.description,
        ...(event.hostGenres ?? []),
    ...
    return eventText.includes(query.toLowerCase());
}
...
export default function SearchClient({ 
    ...
    let filteredEvents = searchQuery === ''
        ? safeEvents
        : safeEvents.filter(event => eventMatchesQuery(
            event,
            searchQuery,
    ...
    let filteredVenues = searchQuery === ''
        ...
        : safeVenues.filter((venue) => {
            const venueText = [venue.name, venue.type, venue.address, venue.bio]
                ...
                .join(' ')
                .toLowerCase();
            return venueText.includes(searchQuery.toLowerCase());
        });
